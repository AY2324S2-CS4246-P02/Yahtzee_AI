...
class GreedyAgent(Agent):
    ...
    def __init__(
        ...
        super().__init__(game, rounds)
        self.name = name
        self.turn = 0
        self.strategy_mapping = {
            0: lambda dice: self.__of_a_kind_strat(dice),
            1: lambda dice: self.__of_a_kind_strat(dice),
            2: lambda dice: self.__full_house_strat(dice),
            3: lambda dice: self.__straight_strat(dice),
            4: lambda dice: self.__straight_strat(dice),
            5: lambda dice: self.__yahtzee_strat(dice),
            6: lambda dice: self.__chance_strat(dice),
        }
...
if __name__ == '__main__':
    ...
    for _ in tqdm(range(MAX_ITER)):
        agent = GreedyAgent(Yahtzee_7.Yahtzee(), 7)
    ...
    history = pd.DataFrame(history, columns = ['Score'] + Yahtzee_7.CATEGORIES_NAMES)
    history.to_csv(os.path.join(os.path.dirname(__file__), '..', 'results', 'greedy_agent_7.csv'))
